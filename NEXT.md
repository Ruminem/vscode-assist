...
- **clangd 가 그 편집을 초기 응답에 담는 것은 확인됨**(clangd 18, `editsNearCursor: true`).
  ...
  맡으므로 급하지 않다.
- **clangd 탐침 하네스를 저장소에 넣을지 정할 것.** 이번에 `unique_ptr` 버그를 잡은 게 이것이고,
  지금은 세션 scratchpad 에만 있어 컨테이너와 함께 사라진다. `tools/probe-clangd.js` 로 넣으면
  픽스처 자리마다 `arrow + plain` 개수를 찍어 기대값을 손이 아니라 측정으로 뽑을 수 있다.
  **넣는 쪽을 권한다** — 이 저장소의 첫 줄이 "provider 동작을 추측하지 말 것, 재고 짤 것"이고,
  재는 도구가 저장소 밖에 있으면 그 규칙이 다음 세션에서 지켜지지 않는다. 단, 돌리려면 clangd 가
  있어야 하므로 없을 때 곱게 빠지게 만들어야 한다.
