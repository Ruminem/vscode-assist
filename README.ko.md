...
`Alt+G`, `Shift+Alt+S`, 함수 이동은 누를 때마다 출력 패널의 **Assist** 채널에 한 줄씩 남김 — 단계마다
언제 끝났고 답이 몇 개였는지, 그리고 총 시간. `Alt+G`는 공급자, 기반 가상 함수 찾기, 이름 검색(안 기다렸으면
...
단계마다 하나씩 따로 잰 시간(채널로는 안 보이는 것, 곧 메뉴가 뜨기 전에 VS Code가 코드 액션을 모으는 시간
포함), 그리고 최근 기록 50줄. 저장하기 전엔 어디에도 안 씀. 파일 경로, 심볼 이름, 커서 줄의 코드가 들어
