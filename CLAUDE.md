...
| `features/note.js` | `assist.saveNote`. 사용자 한 줄 + 언어 서버 버전·설정 + 커서 위치에서 단계별로 하나씩 잰 시간(`round-trip.js`의 `measure`) + 최근 기록을 저장 안 된 마크다운으로 엶. 업무 PC에서 겪은 걸 집에서 고칠 때 넘기는 용도. 키 없음 |
...
0.5.0은 기본 꺼짐이었음 — 이미 깔아 쓰는 사람에게 업데이트만으로 입력이 바뀌는 동작이 켜지지 않게 하려던 것임. 0.5.1에서
