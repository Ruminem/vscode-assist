...
| `tools/check-enum-values.js` | `enum-values.js` 의 글자 판정(`valueOf`·`label`·`literalBase`) 검사. `npm run check-enum-values`. hover 문자열은 clangd 22·cpptools 1.34 실측을 옮긴 것임. **16진수 소문자·8진수 처리 빼기·음수 hex·`=` 앞 고정 빼기·cpptools 형식 빼기, 변이 다섯이 전부 실패하는 것을 확인했음** |
