...
| `tools/check-find-database.js` | `text-guess.js` 의 `locateDatabase` 검사. `npm run check-find-database`. 임시 폴더에 프로젝트를 만들어 세 자리와 우선순위를 봄. **플래그 무시 + `.clangd` 를 옛 탐색 뒤로 돌리면 7건이 실패하는 것을 확인했음** |
